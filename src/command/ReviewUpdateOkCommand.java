...
public class ReviewUpdateOkCommand implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		if (ifNew) {
			if (review_brd_title != null && review_brd_title.length() > 0 && !review_brd_title.trim().equals("")
			&& review_brd_content != null && review_brd_content.length() > 0 && !review_brd_content.trim().equals("")) {
				try {
					cnt = dao.insertReview(mb_uid, class_uid, review_brd_title, review_brd_content);
					if (cnt == 1) cnt = 1;
					if (cnt == 0) cnt = 0;
